...
describe('Badge component', () => {
  it('renders status text and variant classes', () => {
    ...
    expect(screen.getByText('Đã thanh toán')).toHaveClass('text-accent')

    rerender(<Badge variant="unpaid">Chưa đóng</Badge>)
    expect(screen.getByText('Chưa đóng')).toHaveClass('text-danger')

    rerender(<Badge variant="pending">Chờ xác nhận</Badge>)
    expect(screen.getByText('Chờ xác nhận')).toHaveClass('text-warn')
  })
