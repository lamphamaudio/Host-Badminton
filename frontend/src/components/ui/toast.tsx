...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-3 overflow-hidden rounded-2xl border p-4 pr-6 shadow-xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-top-full sm:data-[state=closed]:slide-out-to-bottom-full data-[state=open]:slide-in-from-top-full sm:data-[state=open]:slide-in-from-bottom-full backdrop-blur-md',
  {
    variants: {
      variant: {
        default: 'border-line bg-surface/95 text-fg',
        success: 'border-accent/30 bg-accent/10 text-accent shadow-black/40',
        error: 'border-danger/30 bg-danger/10 text-danger shadow-black/40',
        info: 'border-info/30 bg-info/10 text-info shadow-black/40',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-lg border border-line-strong bg-transparent px-3 text-xs font-bold text-fg hover:bg-raised transition-colors',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-full p-1 text-fg-subtle opacity-0 transition-opacity hover:text-fg group-hover:opacity-100 focus:opacity-100',
      className
    )}
    toast-close=""
    {...props}
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn('text-sm font-bold text-fg leading-tight', className)}
    {...props}
  />
))
